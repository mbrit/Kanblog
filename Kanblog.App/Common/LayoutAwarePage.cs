    ...
    public class LayoutAwarePage : Page, IViewModelHost
    {
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            if (this.DataContext is ViewModel)
                ((ViewModel)this.DataContext).Activated(e.Parameter);
        }
        ...
        protected void InitializeModel(ViewModel model)
        {
            // set...
            this.DataContext = model;
        }

        public void ShowView<T>(object args) 
            where T : ViewModel
        {
            // hack...
            if (typeof(T) == typeof(StoryPageViewModel))
                this.Frame.Navigate(typeof(StoryPage), args);
            else
                throw new NotSupportedException(string.Format("Cannot handle '{0}'.", typeof(T)));
        }
        ...
        public async Task<bool> ConfirmAsync(string message = "Are you sure?")
        {
            bool yes = false;
            var dialog = new MessageDialog(message);
            dialog.Commands.Add(new UICommand("Yes", (cmd) => yes = true));
            dialog.Commands.Add(new UICommand("No", (cmd) => yes = false));
